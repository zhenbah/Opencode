...
// Config holds the orchestrator configuration needed by kubernetes manager
type Config struct {
	Namespace  string
	Kubeconfig string
	Image      string
...
func NewKubernetesPodManager(config *Config) (*KubernetesPodManager, error) {
	...
	client, err := kubernetesclient.NewForConfig(kubeConfig)
	if err != nil {
