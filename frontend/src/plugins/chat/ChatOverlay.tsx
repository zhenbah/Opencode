...
export function ChatOverlay() {
  ...
  // WebSocket connection management - No longer needed for persistent connection
  // The WebSocket upgrade happens during the POST request to /generateScene
  const connectWebSocket = () => {
    console.log('WebSocket connection will be established during message send');
  ...
  const sendMessageToBackend = async (payload: MessagePayload) => {
    ...
    // No need to check WebSocket connection - it will be established during the POST request
    ...
    try {
      // Check if already connected, otherwise create new connection
      if (isConnectedRef.current && wsConnection) {
        console.log('Using existing WebSocket connection');
        wsConnection.send(JSON.stringify({ prompt: contextualMessage }));
        setMessages(prev => prev.filter(msg => msg.id !== statusMessage.id));
      } else {
        console.log('Creating new WebSocket connection to:', 'ws://localhost:3000/ws');
        ...
        ws.onopen = () => {
          ...
          ws.send(JSON.stringify({ prompt: contextualMessage }));
          setMessages(prev => prev.filter(msg => msg.id !== statusMessage.id));
        };
        ...
        ws.onerror = (error) => {
          ...
          setMessages(prev => [
            ...prev.filter(msg => msg.id !== statusMessage.id),
            {
              id: Date.now().toString(),
              username: 'System',
              text: 'WebSocket connection failed. Is the server running?',
              timestamp: new Date(),
              type: 'system'
            }
          ]);
      ...
      setMessages(prev => [
        ...prev.filter(msg => msg.id !== statusMessage.id),
        {
          id: Date.now().toString(),
          username: 'System',
          text: `Error: ${error instanceof Error ? error.message : 'Failed to handle WebSocket connection'}`,
          timestamp: new Date(),
          type: 'system'
        }
      ]);
